\documentclass{diploma}

\student{Ефименко Кирилл Игоревич}
\group{М8О-409Б-22}
\theme{Система индексации блокчейн данных}

\supervisor{Филимонов Николай Сергеевич}
\firstConsultant{Булакина Мария Борисовна}

\faculty{№ 8 <<Компьютерные науки и прикладная математика>>}
\department{806}
\speciality{01.03.02 <<Прикладная математика и информатика>>}
\profile{Информатика}

\departmentFullName{№ 806}
\headOfDepartment{Крылов Сергей Сергеевич}

% Дата. Оставляем пустое место для дня
\date{\uline{\hspace{24pt}} мая \the\year\ года}

\input{contents/glossary}
\input{contents/terms}

\addbibresource{main.bib}

\begin{document}
    \maketitle

    % \includepdf[pages=-]{extra/task} % Задание
    \setcounter{page}{2} % Устанавливает счётчик страниц

    \include{contents/0-abstract} % Реферат

    \tableofcontents % Содержание 
    \termsanddefenitions % Термины и определения
    \listofabbreviations % Перечень сокращений и обозначений
 
    \input{contents/1-introduction} % Введение

    \input{contents/2-00-compilation} % Основная часть
    \input{contents/2-01-titlepage}
    \input{contents/2-02-techtask}
    \input{contents/2-03-abstract}
    \input{contents/2-04-glossary}
    \input{contents/2-05-figures}
    \input{contents/2-06-citations}
    \input{contents/2-07-tables}
    \input{contents/2-08-enums}
    \input{contents/2-09-formulas}
    \input{contents/2-10-math}
    \input{contents/2-11-algorithms}
    \input{contents/2-12-listings}
    
    \input{contents/3-conclusion} % Заключение

    \printbibliography % Список литературы

    \appendix % Приложения
    \include{contents/4-appendix-a}
    \include{contents/4-appendix-b}
    \include{contents/4-appendix-d}
\end{document}
